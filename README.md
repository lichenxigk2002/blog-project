...
这个博客系统，以前端为主场，体验和工程化拉满。不是那种“能跑起来就行”的小玩具，而是我一行行打磨出来、追求极致体验和可维护性较高的新型项目。
...
- /components：几十个高复用组件，动画、响应式、主题切换全支持，什么AI聊天、评论、目录、弹窗、骨架屏、分页、表情、打字机、代码高亮、友链、头像预览……

- /pages：Next.js 路由自动映射....我没用哈，主站、后台、404、首页、文章、归档、友链、相册、灵感、统计、AI 聊天、搜索、标签、留言板……
...
- HTTP 封装：自研 HTTP工具，支持全局 BASE_URL、自动带 token、错误统一处理，具备五大拦截器，分别是 请求拦截、响应拦截、错误拦截、请求取消拦截、请求重试拦截，API 层全部基于 http 封装，接口变动一处改全站生效。
...
- 功能丰富但不臃肿：文章、标签、友链、相册、公告、AI 聊天、评论、后台管理、权限、邮箱通知、云存储、统计分析……。

- 细节体验：智能文章目录、评论区 Markdown/表情/楼中楼、AI 聊天流式响应、操作提示弹窗、主题切换、后台权限分明、性能监控……。
